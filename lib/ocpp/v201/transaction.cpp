...
int32_t EnhancedTransaction::get_seq_no() {
    this->seq_no += 1;
    this->database_handler->update_transaction_seq_no(this->transactionId, this->seq_no);
...
void EnhancedTransaction::update_charging_state(const ChargingStateEnum charging_state) {
    ...
    database_handler->update_charging_state(this->transactionId, charging_state);
}

void EnhancedTransaction::update_sequence_number(const int32_t seq_no) {
    this->database_handler->update_transaction_seq_no(this->transactionId, seq_no);
}
